...
let gainNode = null;
let freqData = null;
let beatEnergy = 0;      // smoothed low-freq energy 0..1 (snappy burst)
let beatPeak = 0;        // dynamic peak tracker for normalisation
let beatSmooth = 0;      // extra-smooth value for gentle ambient sway

function initAudioAnalyser() {
  if (audioCtx) return;

  // Local file:// protocol blocks Web Audio media element readers due to CORS security policies.
  // We disable the analyser locally under file:// so the song plays directly through the element.
  if (window.location.protocol === 'file:') {
    console.warn(
      '🍂 Beat-reactive leaves are disabled locally under file:// due to browser CORS security restrictions.\n' +
      'To test the beat interaction locally, please run a local server (e.g. "python -m http.server 8000") or test on GitHub Pages!'
    );
    return;
  }
  ...
  try {
    ...
    // Connect themeAudio media element source to analyser and output
...
function updateBeat() {
  if (themeAudio && !themeAudio.paused && analyser && freqData) {
  ...
  // Fallback if Spotify is playing live instead of themeAudio:
  if (typeof currentPlaybackState !== 'undefined' && currentPlaybackState.isPlaying) {
    const bpmMs = 520;
    const beatPhase = (performance.now() % bpmMs) / bpmMs;
    const pulse = Math.pow(Math.max(0, 1 - beatPhase * 1.8), 2) * 0.45;
    beatEnergy = beatEnergy * 0.65 + pulse * 0.35;
    beatSmooth = beatSmooth * 0.92 + pulse * 0.08;
    return;
  }

  // Decay smoothly to zero when no music is playing
  beatEnergy = beatEnergy * 0.85;
  beatSmooth = beatSmooth * 0.92;
}
...
function drawLeaves(dt, t) {
  ...
  const speedBoost = 1 + beatBurst * 2.8;               // up to 3.8× fall velocity on beat
  const spinBoost = 1 + beatBurst * 4.0;                // extra spin rotation on beat
  const wobbleBoost = 1 + beatAmbient * 1.4;             // sway amplitude modulation
  const scaleBoost = 1 + beatBurst * 0.18;              // subtle rhythmic size pulse
  ...
  for (let i = 0; i < leaves.length; i++) {
    ...
    const opacityPulse = leaf.opacity + leafBeat * 0.22 * (1 - leaf.opacity);
    ctx.globalAlpha = Math.min(opacityPulse * leaf.depth, 1);
